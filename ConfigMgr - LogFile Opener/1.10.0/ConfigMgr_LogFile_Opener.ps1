...
Function Get-IPAddressString {
    ...
    Try {
        ...
        Else {
            ...
            $ipAddress = Resolve-DnsName $hostname | Where-Object {$_.Type -eq "A"} | Select -ExpandProperty IPAddress -First 1
        }

        # Determine adapter type
        If ($hostnameIsRemote) {
            $netAdapterName = Get-CimInstance -ComputerName $hostname -ClassName Win32_NetworkAdapterConfiguration | Where-Object {$_.IPAddress -eq $ipAddress} -ErrorAction SilentlyContinue | Select -ExpandProperty Description
            $adapterTypeID = Get-CimInstance -ComputerName $hostname -Namespace "root/WMI" -Class MSNdis_PhysicalMediumType -ErrorAction SilentlyContinue | Where-Object {$_.InstanceName -eq $netAdapterName} | Select -ExpandProperty NdisPhysicalMediumType
        }
        Else {
            $netAdapterName = Get-CimInstance -ClassName Win32_NetworkAdapterConfiguration | Where-Object {$_.IPAddress -eq $ipAddress} -ErrorAction SilentlyContinue | Select -ExpandProperty Description
            $adapterTypeID = Get-CimInstance -Namespace "root/WMI" -Class MSNdis_PhysicalMediumType -ErrorAction SilentlyContinue | Where-Object {$_.InstanceName -eq $netAdapterName} | Select -ExpandProperty NdisPhysicalMediumType
        }

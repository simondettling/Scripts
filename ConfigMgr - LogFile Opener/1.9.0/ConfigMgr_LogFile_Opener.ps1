...
Function Get-OperatingSystemData {
    ...
    Try {
        ...
        If ($hostnameIsRemote) {
            $cimObject = Get-CimInstance -ComputerName $hostname -ClassName 'Win32_OperatingSystem' -Property Caption,Version,OSArchitecture,LastBootUpTime  -ErrorAction SilentlyContinue
        }
        Else {
            $cimObject = Get-CimInstance -ClassName 'Win32_OperatingSystem' -Property Caption,Version,OSArchitecture,LastBootUpTime -ErrorAction SilentlyContinue
        }

        # Build data Hash Table from WMI
        $data.osString = "$($cimObject.Caption.Replace('Microsoft', '').Trim()) $($cimObject.OSArchitecture) ($($cimObject.Version))"
        $data.lastBootTime = Get-Date $cimObject.LastBootUpTime -Format $dateTimePattern
...
Function Stop-CcmExec {
    If ((Invoke-MessageBox -Message "Do you really want to stop CcmExec Service on $($hostname)?" -Icon Information -Button YesNo) -eq "Yes") {
        Try {
            If ($hostnameIsRemote) {
            ...
            If ($serviceObject.Status -eq "Stopped") {
                Invoke-MessageBox -Message "CcmExec service successfully stopped." -Icon Information
            ...
            Else {
                Invoke-MessageBox -Message "Unable to stop CcmExec Service on $($hostname).`n`nCurrent service status: $($service.Status)"
            }
        }
        Catch {
            # Display error message in case of a failure and return to the client action menu
            $errorMessage = $_.Exception.Message
            Invoke-MessageBox -Message "Unable to stop CcmExec service on $($hostname).`n`n$errorMessage"
        }
    }
}
...
Function Start-CcmExec {
    ...
    Try {
        ...
        If ($serviceObject.Status -eq "Running") {
            Invoke-MessageBox -Message "CcmExec service successfully started." -Icon Information
        ...
        Else {
            Invoke-MessageBox -Message "Unable to restart CcmExec Service.`n`nCurrent service status: $($service.Status)"
        }
    }
    Catch {
        # Display error message in case of a failure and return to the client action menu
        $errorMessage = $_.Exception.Message
        Invoke-MessageBox -Message "Unable to start CcmExec service.`n`n$errorMessage"
    }
}
...
Function Restart-CcmExec {
    ...
    If ((Invoke-MessageBox -Message "Do you really want to restart CcmExec Service on $($hostname)?" -Icon Information -Button YesNo) -eq "Yes") {
        Try {
            If ($hostnameIsRemote) {
            ...
            If ($serviceObject.Status -eq "Running") {
                Invoke-MessageBox -Message "CcmExec service successfully restarted on $($hostname)." -Icon Information
            ...
            Else {
                Invoke-MessageBox -Message "Unable to restart CcmExec Service on $($hostname).`n`nCurrent service status: $($service.Status)"
            }
        }
        Catch {
            # Display error message in case of a failure and return to the client action menu
            $errorMessage = $_.Exception.Message
            Invoke-MessageBox -Message "Unable to restart CcmExec service on $($hostname).`n`n$errorMessage"
        }
    }
}
...
Function Invoke-ConfigurationBaselineEvaluation {
    ...
    Try {
        ...
        If ($baselineCount -eq 1) {
            $messageText = "$baselineCount Configuration Baseline has been reevaluated."
        } Else {
            $messageText = "$baselineCount Configuration Baselines have been reevaluated."
        }
     ...
     Catch {
        # Display error message in case of a failure and return to the client action menu
        $errorMessage = $_.Exception.Message
        Invoke-MessageBox -Message "Unable to reevaluate Configuration Baselines.`n`n$errorMessage"
     }
}
...
Function Invoke-CcmEval {
    ...
    Catch {
        # Display error message in case of a failure and return to the client action menu
        $errorMessage = $_.Exception.Message
        Invoke-MessageBox -Message "Unable to run CcmEval.`n`n$errorMessage"

        Return $false
    }

    Invoke-MessageBox -Message "CcmEval has been executed." -Icon Information
}
...
Function Update-SoftwareUpdateComplianceState {
    ...
    Catch {
        # Display error message in case of a failure and return to the client action menu
        $errorMessage = $_.Exception.Message
        Invoke-MessageBox -Message "Unable to run refresh Software Update Compliance State.`n`n$errorMessage"

        Return $false
    }

    Invoke-MessageBox -Message "Software Update Compliance State has been successfully refreshed." -Icon Information
}
...
Function Invoke-ClientActionMenu {
    ...
    Write-Output ' [20] Reevaluate Endpoint deployment'
    Write-Output ' [21] Reevaluate Endpoint AM policy'
    ...
    Write-Output ' [40] Start CcmExec'
    Write-Output ' [41] Stop CcmExec'
    Write-Output ' [42] Restart CcmExec'
    Write-Output ' [43] Run CcmEval'
    Write-Output ''
    Write-Output ' --- Options -----------------------------------------------'

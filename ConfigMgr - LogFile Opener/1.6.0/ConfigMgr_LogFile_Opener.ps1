...
Function Get-ModelString {
    Try {
        # Get client version from WMI
        If ($hostnameIsRemote) {
            $cimObject = Get-CimInstance -ComputerName $hostname -ClassName 'Win32_ComputerSystem' -Property Manufacturer,Model  -ErrorAction SilentlyContinue
        }
        Else {
            $cimObject = Get-CimInstance -ClassName 'Win32_ComputerSystem' -Property Manufacturer,Model -ErrorAction SilentlyContinue
        }

        Return "$($cimObject.Manufacturer) $($cimObject.Model)"
    }
    Catch {
        Return 'n/a'
    }
